...
@Component({
  selector: 'app-forme-affectation-ic',
  templateUrl: './forme-affectation-ic.component.html',
  styleUrls: ['./forme-affectation-ic.component.scss']
})
export class FormeAffectationIcComponent {
  @Input() risque!: Risque;
  @Input() numCtx!: number;
  ...
  validateMontantPrincipale(): boolean {
    return this.formeAffectation.mntAffectationPrincipale <= this.risque.ic && this.formeAffectation.mntAffectationPrincipale<= this.dossier.soldeRecouvrement;
  }
  ...
  submitForm(risque: Risque) {
    if (!this.validateMontantPrincipale()) {
      Swal.fire({
        position: "center",
        icon: "error",
        title: "Le montant de l'affectation en principal ne doit pas dépasser le montant IC du risque ou le solde de recouvrement.",
        showConfirmButton: true
      });
      return;
    }

    this.formeAffectation.typeAffectation = 'IC';
