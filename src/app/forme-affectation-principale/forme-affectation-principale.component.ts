...
@Component({
  selector: 'app-forme-affectation-principale',
  templateUrl: './forme-affectation-principale.component.html',
  styleUrls: ['./forme-affectation-principale.component.scss']
})
export class FormeAffectationPrincipaleComponent {
  @Input() risque!: Risque;
  @Input() numCtx!: number;
  ...
  validateMontantPrincipale(): boolean {
    return this.formeAffectation.mntAffectationPrincipale <= this.risque.soldePrincipaleRisque && this.formeAffectation.mntAffectationPrincipale <= this.dossier.soldeRecouvrement;
  }
  ...
  submitForm(risque: Risque) {
    if (!this.validateMontantPrincipale()) {
      Swal.fire({
        position: "center",
        icon: "error",
        title: "Le montant de l'affectation en principal ne doit pas dépasser le solde du risque ou le solde de recouvrement.",
        showConfirmButton: true
      });
      return;
    }

    this.formeAffectation.typeAffectation = 'principale';
